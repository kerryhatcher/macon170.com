...
- Program year runs fall–spring, and Pack 170's is four milestones, not the generic Cub Scouting six (all four Cubmaster-confirmed 2026-07-28): **Lego Pinewood Derby & Cookout** (August — free, and the pack's recruiting event in practice, per `docs/calendar.md`), **fall camp**, **Pinewood Derby** (late Jan), **Blue & Gold Banquet** (Feb), which is where the **Arrow of Light Ceremony** — the crossover into Scouts BSA — happens. The pack does **not** hold a separate Join Scouting Night, does **not** sell popcorn, does **not** camp in summer, and does **not** hold the crossover as its own spring event; earlier drafts of this file claimed all four from generic research. `src/data/pack.ts` `annualProgram` is the machine-readable copy, and a calendar event claims a milestone through its `milestone` column.
