...
export const GENERATION_FENCED_PHASES = new Set(['accept_requested', 'discard_requested', 'carbonize_required', 'completed', 'discarded']);
...
function applyEvent(snapshot, entry, inheritedDiagnostics = []) {
  ...
  switch (event.type) {
    ...
    case 'detector_waivers':
      if (!next.generationCanceled && !GENERATION_FENCED_PHASES.has(next.phase)) {
        next.detectorWaivers = [...(next.detectorWaivers || []), ...(Array.isArray(event.waivers) ? event.waivers : [])];
      }
      break;
    case 'agent_phase':
    ...
    case 'checkpoint':
      ...
      }
      {
        const revisionDomain =
          event.revisionDomain === 'publication' || (event.reason === 'variants_progress' && !event.owner) ? 'publication' : 'browser';
        const revisionField = revisionDomain === 'publication' ? 'publicationCheckpointRevision' : 'browserCheckpointRevision';
        const currentRevision = next[revisionField] ?? (revisionDomain === 'browser' ? next.checkpointRevision : 0) ?? 0;
