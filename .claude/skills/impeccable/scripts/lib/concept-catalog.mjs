...
export const SYSTEM_PREFIXES = ['Palette/material:', 'Type/composition:', 'Topology/navigation:', 'Controls/state:', 'Responsive/motion:'];
const BLAND_FORM_RE =
...
export function validateConceptEntry(concept, { existingForms = new Map() } = {}) {
  ...
  if (typeof concept?.lineage !== 'string' || concept.lineage.trim().length < 12 || concept.lineage.trim().length > 200) {
  ...
  if (typeof concept?.spark !== 'string' || concept.spark.trim().length < 80 || concept.spark.trim().length > 320) {
    errors.push(`concept ${id} needs a vivid creative spark of 80–320 characters`);
  }
  if (typeof concept?.webLeverage !== 'string' || concept.webLeverage.trim().length < 20 || concept.webLeverage.trim().length > 240) {
...
export function validateConceptCatalog(catalog, reviewData, { expectedTotal, minimumTotal, requireApprovedMinimum = true } = {}) {
  const errors = [];
  const warnings = [];
