...
export function parseSurfaceBrief(text, filePath = null) {
  ...
  const relatedTargets = Array.isArray(meta.related_targets) ? meta.related_targets.filter((value) => typeof value === 'string') : [];
  return {
    path: filePath,
    text: String(text || ''),
    body: match
...
export function writeSurfaceBrief({ projectRoot = process.cwd(), primaryTarget, relatedTargets = [], body }) {
