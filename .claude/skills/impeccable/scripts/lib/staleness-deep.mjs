...
const HOOK_SCRIPT_MARKERS = ['skills/impeccable/scripts/hook.mjs', 'skills/impeccable/scripts/hook-before-edit.mjs'];
...
export function checkDesignCoverage({ design, designPath, parseDesignMd }) {
  ...
  const missing = ['colors', 'typography', 'components'].filter((section) => !model[section]);
      ...
      if (missing.length) {
        findings.push(
          finding({
            ...
            filePath: rel,
            severity: 'mention',
            summary: `${rel} ignores file path(s) that no longer exist: ` + `${missing.map((entry) => `\`${entry}\``).join(', ')}.`,
            fix:
      ...
      findings.push(
        finding({
          ...
          summary: `Workspace \`${candidate.path}\` ${
            candidate.productStatus === 'inherited' ? 'inherits the repo-root PRODUCT.md' : 'has a PRODUCT.md'
