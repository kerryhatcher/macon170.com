 ...
 */
export const LIVE_TEMPLATE_EXTENSIONS = Object.freeze(['.html', '.jsx', '.tsx', '.vue', '.svelte', '.astro', '.ex', '.heex', '.eex']);
...
export function matchConfiguredExtension(filePath, extensions) {
  ...
  for (const entry of normalizeExtensionEntries(extensions)) {
    if (name.length > entry.ext.length && name.endsWith(entry.ext) && (!best || entry.ext.length > best.ext.length)) {
      best = entry;
    }
  }
  return best;
