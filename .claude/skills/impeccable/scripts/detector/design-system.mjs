...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [path.join(cwd, '.impeccable', 'design.json'), path.join(cwd, 'DESIGN.json'), path.join(contextDir, 'DESIGN.json')];
...
function colorsClose(a, b) {
  if (!a || !b) return false;
  return Math.max(Math.abs(a.r - b.r), Math.abs(a.g - b.g), Math.abs(a.b - b.b)) <= COLOR_CHANNEL_TOLERANCE;
}
...
function parseDesignColor(value) {
  ...
  if (hsl) {
    return hslToRgb(parseFloat(hsl[1]), parseFloat(hsl[2]) / 100, parseFloat(hsl[3]) / 100, hsl[4] !== undefined ? parseFloat(hsl[4]) : 1);
  }
  return null;
}
...
function fontSizeStepStatus(raw, designSystem) {
  ...
  return designSystem.allowedFontSizes.some((entry) => Math.abs(entry.px - px) <= FONT_SIZE_TOLERANCE_PX) ? 'on-ramp' : 'off-ramp';
}
...
export function offRampClampEndpoints(raw, designSystem) {
  ...
  if (!args) return null;
  return [args[0], args[2]].filter((endpoint) => fontSizeStepStatus(endpoint, designSystem) === 'off-ramp');
}
...
function checkFontStack(stack, filePath, line, designSystem, context) {
  ...
  return [
    makeDesignFinding('design-system-font', filePath, `${context}: ${display} is not declared in DESIGN.md typography`, line, {
      ignoreValue: display,
    }),
  ];
}
...
function checkRadiusValue(value, filePath, line, designSystem, context) {
  ...
  for (const token of extractRadiusTokens(value)) {
    ...
    findings.push(
      makeDesignFinding('design-system-radius', filePath, `${context}: ${token} is outside the DESIGN.md rounded scale`, line, {
        ignoreValue: token,
      }),
    );
  }
...
function checkFontSizeValue(value, filePath, line, designSystem, context) {
  ...
  return [
    makeDesignFinding('design-system-font-size', filePath, `${context}: ${token} is off the DESIGN.md type ramp`, line, {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (designSystem.hasFonts) {
      ...
      for (const match of line.matchAll(GOOGLE_FONT_RE)) {
        ...
        for (const familyMatch of url.matchAll(/[?&]family=([^&]+)/g)) {
          ...
          findings.push(
            makeDesignFinding('design-system-font', filePath, `Google Fonts: ${display} is not declared in DESIGN.md typography`, lineNum, {
              ignoreValue: display,
            }),
          );
        }
    ...
    if (designSystem.hasColors) {
      for (const match of line.matchAll(CSS_COLOR_RE)) {
        ...
        findings.push(
          makeDesignFinding('design-system-color', filePath, `Undocumented color ${raw} is outside DESIGN.md colors`, lineNum, {
            ignoreValue: raw,
          }),
        );
      }
  ...
  for (const el of document.querySelectorAll('*')) {
    ...
    if (designSystem.hasColors) {
      ...
      for (const [kind, raw] of colorChecks) {
        ...
        seenColors.add(key);
        findings.push(
          makeDesignFinding('design-system-color', filePath, `${kind} ${label} on ${tag}${sampleText(el)} is outside DESIGN.md colors`, 0, {
            ignoreValue: label,
          }),
        );
      }
...
function dedupeDesignFindings(findings) {
  ...
  for (const item of findings) {
    const key = [item.antipattern, item.line || 0, normalizeFontName(item.ignoreValue || item.snippet || '')].join('\0');
