...
function serializeDesignSystemForBrowser(designSystem) {
  ...
  return {
    ...
    allowedRadii: (designSystem.allowedRadii || []).map((entry) => Number(entry?.px)).filter((px) => Number.isFinite(px)),
...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  const maxCandidates = Number.isFinite(options?.visualContrastMaxCandidates) ? options.visualContrastMaxCandidates : 12;
  ...
  const filtered = candidates.filter(
    (candidate) => !existingLowContrastSelectors.has(candidate.selector) && !browserResolvedSelectors.has(candidate.selector),
  );
...
async function detectUrl(url, options = {}) {
  ...
  const browserScriptPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', 'detect-antipatterns-browser.js');
  let browserScript;
  try {
