...
(function () {
  ...
  function resolveHeroHeadingSizePx(value) {
    ...
    if (clamp) {
      ...
      if (parts.length === 3) {
        const bounds = [simpleLengthPx(parts[0]), simpleLengthPx(parts[2])].filter((candidate) => candidate !== null);
    ...
    while ((blk = blockRe.exec(content)) !== null) {
      ...
      if ((hairlineCount >= 2 && hasPxCell) || hasPxPairCell) {
        return [
          {
            index: blk.index,
            snippet: hairlineCount >= 2 ? 'two-axis grid-line gradient background' : 'px-tiled hairline line-field background',
          },
        ];
      }
    }
  ...
  function scanCssTextForPulsingDot(content) {
    ...
    for (const [selector, decls] of merged) {
      ...
      const w = cssLengthToPx(resolveVarRefs(decls.get('width') || decls.get('inline-size') || '', customProps));
      const h = cssLengthToPx(resolveVarRefs(decls.get('height') || decls.get('block-size') || '', customProps));
  ...
  function isRepeatedKickerCandidate(opts) {
    const { headingTag, headingText, headingFontSize, kickerTag, kickerText, kickerTextTransform, kickerFontSize, kickerLetterSpacing } =
      opts;
  ...
  function checkEmDashOveruseDOM() {
    ...
    const text = typeof body.innerText === 'string' && body.innerText ? body.innerText : body.textContent || '';
  ...
  function checkQuality(opts) {
    ...
    // visual failure mode.
    {
      ...
      if (
        ...
        const anyVisible =
          borderVisible.top || borderVisible.right || borderVisible.bottom || borderVisible.left || outlineVisible || bgVisible;
        if (anyVisible) {
  ...
  function checkElementColors(el, style, tag, window, customPropMap, hasAnchorInheritRule) {
    ...
    let ownBg = (customPropMap ? parseColorResolved(style.backgroundColor, customPropMap) : null) || readOwnBackgroundColor(el, style);
  ...
  function checkElementHeroEyebrow(el, style, tag, window, customPropMap) {
    ...
    return checkHeroEyebrow({
      ...
      siblingHasAccentDashPseudo: typeof window.hasAccentDashPseudo === 'function' ? window.hasAccentDashPseudo(sibling) : false,
    });
  }
  ...
  function isCardLikeDOM(el) {
    ...
    const hasBg =
      (style.backgroundColor && style.backgroundColor !== 'rgba(0, 0, 0, 0)') || /\bbg-(?:white|gray-\d+|slate-\d+)\b/.test(cls);
  ...
  function isCardLike(el, win) {
    ...
    const hasShadow =
      (style.boxShadow && style.boxShadow !== 'none') || /\bshadow(?:-sm|-md|-lg|-xl|-2xl)?\b/.test(cls) || /box-shadow/i.test(rawStyle);
    const hasBorder = /\bborder\b/.test(cls);
  ...
  function isRepeatedTextContainer(style) {
    ...
    const borderSides = ['Top', 'Right', 'Bottom', 'Left'].filter((side) => (parseFloat(style[`border${side}Width`]) || 0) >= 1).length;
  ...
  function checkRepeatedContainerTextDOM() {
    return collectRepeatedContainerTextFindings(document, (el) => getComputedStyle(el), { isVisible: isRenderedForBrowserRule });
  }
    ...
    if (fontSize >= OVERSIZED_H1_FONT_PX && textLen >= OVERSIZED_H1_MIN_CHARS) {
      ...
      if (rect && viewportWidth > 0 && viewportHeight > 0) {
        ...
        const dominatesViewport =
          heightRatio >= OVERSIZED_H1_MIN_VIEWPORT_HEIGHT_RATIO || areaRatio >= OVERSIZED_H1_MIN_VIEWPORT_AREA_RATIO;
        if (!dominatesViewport) return [];
        viewportDetail = `, ${Math.round(heightRatio * 100)}vh`;
      }
      return [
        { id: 'oversized-h1', snippet: `${Math.round(fontSize)}px h1, ${textLen} chars${viewportDetail} "${headingText.slice(0, 60)}"` },
      ];
    }
    return [];
  }
  ...
  function clippingContainerIsIntentionalViewport(el) {
    ...
    return (
      /\b(carousel|comparison|compare|fisheye|marquee|preview|scroller|slider|slideshow|split|viewport)\b/.test(ident) ||
  ...
  function checkElementBlinkingCursorDOM(el) {
    ...
    const blinkName = names.find((n) => /blink|caret|cursor/i.test(n)) || names.find((n) => keyframesToggleVisibilityDOM(n));
    if (!blinkName) return [];
    ...
    if (!glyphCursor) {
      ...
      const hasBorderFill = ['Left', 'Right', 'Bottom'].some((side) => (parseFloat(style[`border${side}Width`]) || 0) >= 1);
    ...
    const inHeroRegion = pageTop <= 900 || !!(el.closest && el.closest('header, nav, [role="banner"], [role="navigation"]'));
    return [
      {
        id: 'blinking-cursor',
  ...
  function checkEdgeFlushCardsDOM() {
    ...
    for (const scroller of document.querySelectorAll('*')) {
      ...
      for (const card of scroller.querySelectorAll('*')) {
        ...
        const borderSides = ['Top', 'Right', 'Bottom', 'Left'].filter((side) => (parseFloat(cs[`border${side}Width`]) || 0) > 0).length;
  ...
  if (IS_BROWSER) {
    ...
    const EXTENSION_MODE =
      (_myScript && _myScript.dataset.impeccableExtension === 'true') || document.documentElement.dataset.impeccableExtension === 'true';
  ...
  `;
    ...
    function pointToImageSource(point, paintedRect) {
      ...
      return {
        x: Math.max(
          0,
          Math.min(paintedRect.intrinsicWidth - 1, ((point.x - paintedRect.left) / paintedRect.width) * paintedRect.intrinsicWidth),
        ),
        y: Math.max(
          0,
          Math.min(paintedRect.intrinsicHeight - 1, ((point.y - paintedRect.top) / paintedRect.height) * paintedRect.intrinsicHeight),
        ),
      };
    }
    ...
    function serializeFindings(allFindings) {
      return allFindings.map(({ el, findings }) => ({
        ...
        rect:
          el !== document.body && el !== document.documentElement && el.getBoundingClientRect ? el.getBoundingClientRect().toJSON() : null,
    ...
    function scheduleLazyVisualContrast(groupMap, analyses, options = {}, runtime = {}) {
      ...
      const unresolved = (analyses || []).filter(
        (result) => result?.status === 'unresolved' && result.reason === 'text outside viewport' && result.selector,
      );
