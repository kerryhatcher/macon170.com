...
Output (JSON):
  ...
  if (priorReceipt) {
    const sameOperation =
      priorReceipt.operation === requestedOperation && (isDiscard || String(priorReceipt.variantId) === String(variantNum));
    console.log(
      JSON.stringify(
        sameOperation
  ...
  if (svelteComponentManifest) {
    ...
    try {
      result = withSourceLockSync(
        path.resolve(process.cwd(), svelteComponentManifest.sourceFile),
        'accept:' + id,
        () => inlineSvelteComponentAccept(svelteComponentManifest, variantNum, paramValues, process.cwd()),
        { waitMs: ACCEPT_LOCK_WAIT_MS },
      );
    } catch (err) {
      result = operationFailure(err, {
...
function manualEditOpAppearsInBlock(op, originalBlock) {
  const candidates = [op?.newText, op?.originalText].filter((text) => typeof text === 'string' && text.length > 0);
 ...
 */
function buildCarbonizeReplacement({ indent, commentSyntax, isJsx, id, variantNum, cssContent, paramValues, restored }) {
  const lines = [];
  if (!cssContent) {
    lines.push(...restored);
    return lines;
