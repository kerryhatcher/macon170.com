...
export const ACK_EXTS = new Set(['.tsx', '.jsx', '.html', '.htm', '.vue', '.svelte', '.astro', '.css', '.scss', '.sass', '.less']);
...
export function resolveProjectCwd(event, fallback = process.cwd()) {
  return event?.cwd || (Array.isArray(event?.workspace_roots) && event.workspace_roots[0]) || envProjectDir(fallback) || fallback;
}
...
function extractMotionIgnoreValue(text) {
  ...
  if (animation) {
    const token = animation[1].split(/[,\s]+/).find((part) => /bounce|elastic|wobble|jiggle|spring/i.test(part));
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) =>
    [header, ...linesArr, ...(omitted ? [`... and more (see ${IMPECCABLE_COMMAND} audit).`] : []), '', footer].join('\n');
...
function formatFindingLine(f) {
  ...
  const ignoreSegment = ignoreCommand ? ` If the user explicitly confirms this value is intentional: \`${ignoreCommand}\`.` : '';
...
export function normalizeHookEvent(event, projectCwd, harness = 'claude') {
  ...
  const cwd = event.cwd || (Array.isArray(event.workspace_roots) && event.workspace_roots[0]) || envProjectDir(projectCwd) || projectCwd;
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    for (const filePath of targetFiles) {
      ...
      const useHtmlEngine = configuredExt ? configuredExt.engine === 'html' : ext === '.html' || ext === '.htm';
      ...
      // knows to re-scan it.
      const { immediate, deferred } = tiered ? splitFindingsByTier(filtered) : { immediate: filtered, deferred: [] };
    ...
    if (freshGroups.length > 0 || deferredTotal > 0 || (cacheDirty && fs.existsSync(path.join(projectCwd, '.impeccable')))) {
