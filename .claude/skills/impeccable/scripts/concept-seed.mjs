...
import { approvedPoolRevision, deterministicRank, readConceptCatalog, validateConceptCatalog, WELL_TIERS } from './lib/concept-catalog.mjs';
...
function loadLocal(catalogDir = CATALOG_DIR) {
  ...
  let localState;
  try {
    const catalogState = readConceptCatalog(join(catalogDir, 'concept-ingredients.json'), join(catalogDir, 'concept-reviews.json'));
...
export function selectApprovedChallengers({ scope, key, reroll = 0, sourceConcepts = null }) {
  ...
  const wanted = scope === 'direction' ? new Set(['world', 'dual']) : new Set(['composition', 'dual']);
  ...
  const pickRound = (round, excluded) => {
    ...
    return tierOrder.flatMap((tier, index) => {
      ...
      const first = order[0];
      const second = order.find((concept) => concept.familyId !== first.familyId) || order.find((concept) => concept.id !== first.id);
...
`;
  ...
  const stagings = Array.isArray(data.stagings) ? data.stagings : data.staging ? [data.staging] : [];
  const stagingBlock =
    stagings.length > 0
    ...
    if (chosenIdx !== -1) {
      ...
      process.stdout.write(
        await renderConceptSeed({
          ...
          key: fromIdx !== -1 ? args[fromIdx + 1] : process.env.IMPECCABLE_CONCEPT_SEED || crypto.randomBytes(4).toString('hex'),
