...
import { applyDeferredSvelteComponentAccepts, removeAllSvelteComponentSessions } from './live/svelte-component.mjs';
...
const DESIGN_MD_PATH = PROJECT_CONTEXT.designPath ? path.resolve(process.cwd(), PROJECT_CONTEXT.designPath) : null;
...
function acknowledgePendingEvent(id, sourceEventType) {
  ...
  const idx = state.pendingEvents.findIndex(
    (entry) => entry.event?.id === id && (!sourceEventType || entry.event?.type === sourceEventType),
  );
...
function releasePendingEvent(id, sourceEventType) {
  const entry = state.pendingEvents.find((item) => item.event?.id === id && (!sourceEventType || item.event?.type === sourceEventType));
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      const jsonPath =
        resolveDesignSidecarPath(process.cwd(), PROJECT_CONTEXT.designContextDir || CONTEXT_DIR) || getDesignSidecarPath(process.cwd());
...
if (args.includes('stop')) {
  ...
  if (!keepInject) {
    ...
    try {
      ...
      const detail = err.stderr?.toString?.().trim?.() || err.stdout?.toString?.().trim?.() || err.message || String(err);
