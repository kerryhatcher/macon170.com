...
function normalizeBatchResult(result) {
  ...
  const appliedEntryIds = Array.isArray(result.appliedEntryIds) ? result.appliedEntryIds.filter((id) => typeof id === 'string') : [];
...
export function chooseCopyEditAgent({ env = process.env, authCheck = commandAuthed, chatAvailable = () => false } = {}) {
 ...
 */
export function describeNoProviderError({ exists = commandExists, chatAvailable = () => false, env = process.env } = {}) {
