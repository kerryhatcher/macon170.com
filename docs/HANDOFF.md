...
*Written 2026-07-26, cutting session short due to usage limits. Read this first when resuming.*
...
- **`docs/Offical-info.md`** — HUMAN-WRITTEN canonical pack/council facts. Never edit without explicit instruction. Overrides research docs on conflict. Confirms: pack meets ~monthly, Tuesdays 6:30 PM at Highland Hills Baptist Church; dens set own times.
...
2. Re-run fact-checks for any doc that wasn't corrected: verify every claim/URL against primary sources (org's own current website ALWAYS outranks aggregators like FaithStreet — that's how the SBC error happened). Agents' briefs emphasized: fix in place, mark unverified claims, flag dead links ⚠️, never revert user corrections, never edit Offical-info.md.
...
4. **Unknown pack facts** (dues, roster, den structure, contact emails, event dates) = loud placeholders in ONE easy-to-edit data file. Meeting info is known (Offical-info.md).
