...
| `links.parentHandbook` | `null`      | **Nothing.** The field is referenced by no page, so it causes no visible placeholder. Surfacing a handbook needs a `/resources/` card as well as a URL — or delete the field if the pack has no handbook.                                                                                                                                             |
