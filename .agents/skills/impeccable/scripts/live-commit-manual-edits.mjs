...
function normalizeFailedEntries(batch, result, fallbackReason) {
  ...
  for (const entry of batch.entries || []) {
    ...
    failed.push({
      ...
      candidates: Array.isArray(item.candidates) && item.candidates.length > 0 ? item.candidates : candidatesForEntry(batch, entry.id),
    });
  }
  return failed;
}
...
function sourceHintWindowFailure(cwd, op) {
  ...
  if (typeof op.originalText === 'string' && op.originalText && lineText.includes(op.originalText) && !lineShowsAppliedOp(lineText, op)) {
    return {
      file: relative,
      line,
...
function coupledObjectKeyFailuresForOp(batch, op, cwd) {
  if (typeof op?.originalText !== 'string' || typeof op?.newText !== 'string' || op.originalText === op.newText) return [];
...
function verificationTargetPassesLines(lines, target, op) {
  ...
  const canSearchWindow =
    target.reported || kind.includes('context_text_match') || kind.includes('object_key_match') || kind.includes('text_match');
...
function verifyAppliedEntry({ batch, entry, reportedFiles, cwd }) {
  ...
  for (const rawOp of entry.ops || []) {
    ...
    if (coupledObjectKeyFailures.length === 0 && targets.some((target) => verificationTargetPasses(cwd, target, op))) continue;
...
function changedFilesSinceSnapshot(cwd, snapshot, scopeFiles = null) {
  ...
  const scopedFiles =
    Array.isArray(scopeFiles) && scopeFiles.length > 0 ? scopeFiles.map((file) => normalizeRollbackPath(cwd, file)).filter(Boolean) : null;
...
function rollbackChangedFiles(cwd, snapshot, extraFiles = [], scopeFiles = []) {
  const scope = new Set([...(scopeFiles || []), ...(extraFiles || [])].map((file) => normalizeRollbackPath(cwd, file)).filter(Boolean));
...
export async function commitManualEdits({
  ...
  if (result.status === 'error') {
    ...
    return {
      applied: [],
      failed: failed.length > 0 ? failed : verificationFailuresForEntries(batch, batch.entries, result.message || 'AI copy edit failed'),
      files: result.files || [],
      cleared: 0,
      count,
      pageUrl,
  ...
  const unreportedEntries =
    result.status === 'done' || result.status === 'partial'
      ...
      : [];
  const nonRepairFailed = [...verificationFailuresForEntries(batch, unreportedEntries, 'not_reported_applied'), ...aiFailed];
  ...
  if (!postChecks.ok) {
    const postCheckEntries =
      verifiedAppliedIds.length > 0 ? reportedAppliedEntries.filter((entry) => verifiedAppliedIds.includes(entry.id)) : batch.entries;
    return repairPostApplyValidation({
      batch,
      cwd,
      pageUrl,
      ...
      chatAvailable,
      transactionId,
      appliedEntryIds: verifiedAppliedIds.length > 0 ? verifiedAppliedIds : postCheckEntries.map((entry) => entry.id).filter(Boolean),
      files: result.files || [],
      failed,
      notes: result.notes || [],
