...
import { buildStalenessDirective, filterFreshFindings, stalenessCheckDisabled } from './lib/staleness-notice.mjs';
...
const UPDATE_CACHE_PATH = process.env.IMPECCABLE_UPDATE_CACHE || path.join(os.homedir(), '.impeccable', 'update-check.json');
...
export function loadContext(cwd = process.cwd(), options = {}) {
  ...
  const surfaceResolution = resolveSurfaceBrief(resolved.projectRoot, hasTargetOption(options) ? options.targetPath : null);
...
function resolveContext(cwd = process.cwd(), options = {}) {
  ...
  const rootContextDir = project.repoRoot !== project.projectRoot ? resolveLocalContextDir(project.repoRoot) : null;
  ...
  return {
    contextDir: productPath ? path.dirname(productPath) : designPath ? path.dirname(designPath) : envContextDir || project.projectRoot,
    productPath,
    designPath,
...
function nearestProjectLikeRoot(repoRoot, targetDir) {
  ...
  while (dir && dir !== stop) {
    if (firstExisting(dir, [...PRODUCT_NAMES, ...DESIGN_NAMES]) || fs.existsSync(path.join(dir, 'package.json'))) {
      return dir;
    }
    const parent = path.dirname(dir);
    if (parent === dir) break;
...
function workspacePatternMatchesRel(pattern, relSegments) {
  ...
  if (patternSegments.includes('**')) {
    ...
    const literalPrefix = firstGlobIndex === -1 ? patternSegments : patternSegments.slice(0, firstGlobIndex);
...
function readProjectPatternGroups(repoRoot) {
  return [
    ...
    [...readPackageWorkspaces(repoRoot), ...readPnpmWorkspaces(repoRoot), ...readLernaWorkspaces(repoRoot)].filter(Boolean),
  ];
}
...
function loadNativePlatformReferences(platform) {
  const names = platform === 'adaptive' ? ['ios', 'android'] : platform === 'ios' || platform === 'android' ? [platform] : [];
  ...
  const inspectFile = (filePath) => {
    ...
    if (!['.html', '.htm'].includes(ext) && evidence.length > 300) {
      ...
      const embeddedVisualDeclarations =
        evidence.match(/\b(?:color|background(?:-color)?|border(?:-color)?|font-family)\s*:/gi)?.length ?? 0;
      const classTokens = [...evidence.matchAll(/class(?:Name)?\s*=\s*["'`]([^"'`]+)["'`]/gi)].reduce(
        (count, match) => count + match[1].trim().split(/\s+/).length,
        0,
      );
      if ((embeddedCustomProperties >= 3 && embeddedVisualDeclarations >= 3) || embeddedVisualDeclarations >= 5 || classTokens >= 12)
        return true;
    }
...
export function extractPlatform(product) {
  ...
  if (tokens.length >= 2 && tokens.every((t) => t === 'ios' || t === 'android') && tokens.includes('ios') && tokens.includes('android')) {
    return 'adaptive';
  }
  return null;
}
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/impeccable/scripts/hook.mjs') || value.includes('skills/impeccable/scripts/hook-before-edit.mjs');
  }
