...
export function resolveLiveTarget(cwd = process.cwd(), args = []) {
  ...
  const absoluteTargetPath = targetPath ? (path.isAbsolute(targetPath) ? targetPath : path.resolve(originalCwd, targetPath)) : null;
  const projectRoot = targetPath ? resolveProjectRoot(originalCwd, { targetPath: absoluteTargetPath }) : originalCwd;
  return {
    originalCwd,
    projectRoot,
    targetPath,
