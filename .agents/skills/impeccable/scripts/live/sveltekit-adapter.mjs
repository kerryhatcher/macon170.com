...
export function detectSvelteKitProject(cwd = process.cwd(), config = null) {
  ...
  const hasTemplateMarkers =
    fileIncludes(path.join(cwd, appHtml), '%sveltekit.body%') && fileIncludes(path.join(cwd, appHtml), '%sveltekit.head%');
...
export function buildSvelteLiveRootComponent(port, token) {
  const liveUrl = 'http://localhost:' + Number(port) + '/live.js' + (token ? '?token=' + encodeURIComponent(token) : '');
  return `<script>
