    ...
    if (p === '/manual-edit-stash' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        recordManualEditActivity('manual_edit_stashed', {
          ...
          pendingCount,
          totalCount,
          hintedFileCount: new Set((msg.ops || []).map((op) => summarizeManualLogFile(op.sourceHint?.file, projectCwd())).filter(Boolean))
            .size,
        });
      ...
      recordManualEditActivity('manual_edit_discarded', {
        ...
        transactionRollback: transactionRollback
          ? {
              ...
              rolledBackFiles:
                transactionRollback.rolledBackFiles?.map((file) => summarizeManualLogFile(file, projectCwd())).filter(Boolean) || [],
