...
import { applySvelteKitLiveAdapter, detectSvelteKitProject, removeSvelteKitLiveAdapter } from './live/sveltekit-adapter.mjs';
import { applyTanStackLiveAdapter, detectTanStackStartProject, removeTanStackLiveAdapter } from './live/tanstack-adapter.mjs';
...
Output (JSON):
  ...
  const gitIgnore = ensureLiveGitIgnores(process.cwd(), nuxt ? [nuxt.pluginFile] : tanstack ? [tanstack.componentFile] : []);
...
export function ensureLiveGitIgnores(cwd = process.cwd(), extraPatterns = []) {
  ...
  const block = [IGNORE_MARKER_OPEN, ...new Set([...LIVE_IGNORE_PATTERNS, ...extraPatterns]), IGNORE_MARKER_CLOSE].join('\n');
...
export function detectNuxtProject(cwd = process.cwd()) {
  const configFile = fs
    ...
    .find((entry) => entry.isFile() && /^nuxt\.config\.(?:js|mjs|cjs|ts|mts|cts)$/.test(entry.name))?.name;
  ...
  } else if (fs.existsSync(path.join(cwd, 'app', 'app.vue')) || fs.existsSync(path.join(cwd, 'app', 'pages'))) {
    appDir = 'app';
  }
