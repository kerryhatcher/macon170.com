...
export const COMPOSITION_GRAMMAR_PREFIXES = ['Staging/hierarchy:', 'Sequence/attention:', 'Controls/state:', 'Adaptation:'];
...
export function validateCompositionEntry(composition, { existingForms = new Map() } = {}) {
  ...
  if (typeof composition?.lineage !== 'string' || composition.lineage.trim().length < 12 || composition.lineage.trim().length > 200) {
  ...
  if (typeof composition?.spark !== 'string' || composition.spark.trim().length < 80 || composition.spark.trim().length > 320) {
