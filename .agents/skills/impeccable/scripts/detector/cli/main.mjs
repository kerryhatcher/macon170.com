...
import { filterDetectionFindings, readDetectionConfig, shouldIgnoreDetectionFile } from '../../lib/impeccable-config.mjs';
import { HTML_EXTENSIONS, buildImportGraph, detectFrameworkConfig, isPortListening, walkDir } from '../node/file-system.mjs';
...
async function detectCli() {
  ...
  if (args.includes('--gpt') || args.includes('--gemini')) {
    process.stderr.write('Note: --gpt and --gemini are deprecated and ignored. Generated-UI tells now run by default.\n');
  }
  const configEnabled = !args.includes('--no-config');
  const detectionConfig = configEnabled ? readDetectionConfig(process.cwd()) : { ignoreRules: [], ignoreFiles: [], ignoreValues: [] };
  const scopes = [];
  for (let i = 0; i < args.length; i++) {
    ...
    if (parsed.length === 0) {
      process.stderr.write(`Error: --scope requires a value. Valid scopes: ${[...RULE_SCOPES].join(', ')}\n`);
      process.exit(1);
    }
    scopes.push(...parsed);
  ...
  if (unknownScopes.length > 0) {
    process.stderr.write(`Error: unknown --scope value(s): ${unknownScopes.join(', ')}. Valid scopes: ${[...RULE_SCOPES].join(', ')}\n`);
    process.exit(1);
  }
    ...
    try {
      for (const target of paths) {
        if (urlRe.test(target)) {
          ...
          // process.cwd()'s.
          const urlOptions = /^file:/i.test(target) ? scanOptionsFor(fileUrlToLocalPath(target)) : baseScanOptions;
          try {
            const scanner = browserDetector ? (url) => browserDetector.detectUrl(url, urlOptions) : (url) => detectUrl(url, urlOptions);
        ...
        if (stat.isDirectory()) {
          ...
          const files = walkDir(resolved).filter((file) => !shouldIgnoreDetectionFile(file, process.cwd(), detectionConfig));
